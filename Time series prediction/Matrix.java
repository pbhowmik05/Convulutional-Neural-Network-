		...
		try
		{
			...
			while (s.hasNextLine())
			{
				...
				else if (upper.startsWith("@ATTRIBUTE"))
				{
					...
					sp.advance(10);
					sp.skipWhitespace();
					sp.advance(1); //  added this line
                    String attrName = sp.until('\''); // modified this line . String runs until the next single quote, rather than white space
                    sp.advance(1); // added this line
					m_attr_name.add(attrName);
					sp.skipWhitespace();
					int valCount = 0;
					if(sp.pos < sp.str.length() && sp.peek() == '{')
					{
						sp.advance(1);
						while(sp.peek() != '}')
						{
